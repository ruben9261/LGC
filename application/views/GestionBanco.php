...
<html> 
  <head> 
    <title>Gesti&oacute;n de Banco</title> 
...
<body> 
    ...
    <div class="container">
         ...
         <P class="text-center" style="color:rgb(0,128,255);"> 
		        GESTI&Oacute;N BANCO
		 </P>
         <br/>
    	<div class="well" style="background-color:rgb(255,255,255); border: 1px solid rgb(0,128,255);">    	     
		     
		    <form   id="frmGestBanco"  action="<?php echo base_url()?>mantBanco_c/mostrar_Nuevo" method="POST" > 
...
{  $.ajax({
		    url:  "<?php echo base_url()?>mantBanco_c/eliminar",
	    	type: 'post',//metodo
	    	data: { codigo:  cod
	    		  }, //parametros
        			...
        			contendiopag="<span>Resultado:</span>";
          		   ...
          		   if (tpaginas<=10)
    				    { for(var x=1;x<=tpaginas;x++) 
    				      { contendiopag=contendiopag+"<li><a id='"+x+"' href='#' onclick="+"bus_banco('"+x+"')>"+x+"</a></li>";							   
    				      }
    				    }
    				if (tpaginas>10)
        			    {if (npagina>=1)
            			    {  var id=npagina-5;  	      
    						   for(var x=1; x<=5;x++)
        						   {   id++;
    								     contendiopag=contendiopag+"<li><a id='"+id+"'  href='#' onclick="+"bus_banco('"+id+"'>"+id+"</a></li>";
    							   }
    						     var id=npagina;  	      
    							 for(var x=1; x<=5;x++)
        							 {  id++;
           	  	 ...
           	  	 {  
                          ...
                          {  var fila=lista[i];
                             ...
                             sb=sb+"   <div class='col-xs-8'  style='background-color:#fff; border: 1px solid rgb(0,128,255); font-size:10px;'> ";
   	                          ...
   	                          sb=sb+"	<a href='<?php echo base_url('mantBanco_c/mostrar_Editar')?>"+"/"+fila['COD_BANCO'].toString()+"'>";
      	  			    	  sb=sb+"	   Editar&nbsp";
      	  					  sb=sb+"	   <img src='<?php echo base_url()?>public/images/EDITAR.png'>";
      	  					  sb=sb+"	</a>"; 
	   	  			 	      sb=sb+"  </div>";
	   	  			 	      sb=sb+"  <div class='col-xs-2'  style='background-color:#fff; border: 1px solid rgb(0,128,255); font-size:10px;'>";
	   	  			 	      sb=sb+"	    <a href='#' onclick=fn_eliminar('"+fila["COD_BANCO"].toString()+"')>";
	   	  			    	  sb=sb+"	       Eliminar&nbsp";
      ...
      <div class="modal-dialog">
        <div class="modal-content">
           <div class="modal-header">
               <a  class="close" data-dismiss="modal"  onclick="bus_banco(1);">&times;</a>
               <h4 class="modal-title" >Eliminaci&oacute;n de Banco</h4>
           </div>
           <div class="modal-body">
